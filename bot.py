...
# Mobile headers for initial request
MOBILE_HEADERS = {
...
class EarnCryptoBot:
    def __init__(self):
        ...
        self.verify_url = ""
        self.first_request_done = False  # Track if initial request with params is done
        
        self.config = load_config()
        self.user_agent = "Mozilla/5.0 (Linux; Android 10; M2006C3LG Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.7778.217 Mobile Safari/537.36"
    ...
    def headers(self, extra=None):
        h = {
            ...
            'Accept': MOBILE_HEADERS.get('Accept', 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'),
            'sec-ch-ua': MOBILE_HEADERS.get('sec-ch-ua', '"Chromium";v="148", "Android WebView";v="148", "Not/A)Brand";v="99"'),
            'sec-ch-ua-mobile': MOBILE_HEADERS.get('sec-ch-ua-mobile', '?1'),
            'sec-ch-ua-platform': MOBILE_HEADERS.get('sec-ch-ua-platform', '"Android"'),
            'upgrade-insecure-requests': MOBILE_HEADERS.get('upgrade-insecure-requests', '1'),
            'sec-gpc': MOBILE_HEADERS.get('sec-gpc', '1'),
            'dnt': MOBILE_HEADERS.get('dnt', '1'),
            'x-requested-with': MOBILE_HEADERS.get('x-requested-with', 'mark.via.gp'),
            'sec-fetch-site': MOBILE_HEADERS.get('sec-fetch-site', 'none'),
            'sec-fetch-mode': MOBILE_HEADERS.get('sec-fetch-mode', 'navigate'),
            'sec-fetch-user': MOBILE_HEADERS.get('sec-fetch-user', '?1'),
    ...
    def make_initial_request(self):
        """Make the initial request with 'r' parameter before login"""
        try:
            bot.info("Making initial connection request...")
            params = {'r': '6517'}
            
            # Use mobile headers for initial request
            initial_headers = {
                'User-Agent': self.user_agent,
                **MOBILE_HEADERS
            }
            ...
            if 'Just a moment...' in response.text or response.status_code == 403:
                ...
                run_cloudflare_bypass(self.session, f"{self.base}?r=6517")
                time.sleep(2)
                # Retry with bypass
                response = self.session.get(
                    f"{self.base}",
            ...
            bot.error(f"Initial request failed: {str(e)}")
            return False
    ...
    def login(self):
        # First, make the initial request with 'r' parameter
        if not self.first_request_done:
            if not self.make_initial_request():
                return False
    ...
    def run(self):
        ...
        # Make initial request before login
        bot.info("Initiating connection with mobile parameters...")
        if not self.make_initial_request():
            bot.error("Initial connection failed!")
            return
