...
def load_config():
    ...
    return {
        "wallet": "",
        "api_key": "",
        "solver_mode": "api",  # api, seledroid_fullscreen, seledroid_interstitial
...
class AltcrypBot:
    ...
    def _generate_fp_data(self):
        canvas_data = "FaucetSpy_Track_2026_Secure_v2"
        components = [
            canvas_data,
            "360x800",
            "24",
        ...
        fp_token = "HW_" + str(abs(hash_val))
        
        return {
            'fp_device_token': fp_token,
            'fp_os_name': 'Android',
            'fp_browser_name': 'Chrome',
            'fp_screen_res': '360x800',
            'fp_user_timezone': 'Asia/Colombo',
            'fp_browser_lang': 'en-GB',
            'fp_cpu_cores': '8',
...
def show_set_solver_menu(config):
    ...
    while True:
        ...
        # Option 1: BypassAllShortLinks API
        print(f"  {C['menu']}[1]{C['reset']} {SOLVER_MODES['api']}")
        
        # Option 2: Seledroid (if available)
