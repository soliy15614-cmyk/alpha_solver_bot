...
def load_config():
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            pass
    return {"user_agent": "", "wallet_email": "", "api_key": "", "api_provider": ""}
...
class EarnCryptoBot:
    def __init__(self):
        ...
        self.user_agent = self.config.get("user_agent") or "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    ...
    def show_menu(self):
        while True:
            ...
            ua_status = "on" if self.config.get("user_agent") else "off"
            wallet_status = "on" if self.config.get("wallet_email") else "off"
            api_status = "on" if self.config.get("api_key") else "off"
            
            print(f"  [1] Set User-Agent          [{ua_status}]")
            print(f"  [2] Set Account             [{wallet_status}]")
            print(f"  [3] Set API Key             [{api_status}]")
            print(f"  [4] Start Work")
            print(f"  [5] Exit\n")
    ...
    def set_user_agent(self):
        bot.show_banner("EarnCryptoWRS")
        print(f"  Current UA: {self.config.get('user_agent', 'Not set')[:60]}...")
        print()
        ua = input("  Enter User-Agent (or press Enter for default): ").strip()
        if ua:
            self.config["user_agent"] = ua
            self.user_agent = ua
        else:
            self.config["user_agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
            self.user_agent = self.config["user_agent"]
        save_config(self.config)
        bot.success("User-Agent saved!", "EarnCryptoWRS")
        time.sleep(1.5)
